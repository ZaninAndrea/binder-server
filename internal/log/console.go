...
const ServiceFieldName string = "service_name"
...
func (w ConsoleWriter) writeFields(evt map[string]interface{}, buf *bytes.Buffer) {
	...
	for field := range evt {
		...
		switch field {
		case zerolog.LevelFieldName, zerolog.TimestampFieldName, zerolog.MessageFieldName, zerolog.CallerFieldName, ServiceFieldName:
			continue
		}
...
func (w ConsoleWriter) writePart(buf *bytes.Buffer, evt map[string]interface{}, p string) {
	...
	case ServiceFieldName:
		f = consoleDefaultFormatService(w.NoColor)
	default:
		if w.FormatFieldValue == nil {
...
func consoleDefaultFormatLevel(noColor bool) Formatter {
	traceColor := colorize("TRACE ", colorMagenta, noColor)
	debugColor := colorize("DEBUG ", colorYellow, noColor)
	infoColor := colorize("INFO  ", colorGreen, noColor)
	warnColor := colorize("WARN  ", colorRed, noColor)
	errorColor := colorize(colorize("ERROR ", colorRed, noColor), colorBold, noColor)
	fatalColor := colorize(colorize("FATAL ", colorRed, noColor), colorBold, noColor)
	panicColor := colorize(colorize("PANIC ", colorRed, noColor), colorBold, noColor)
	unknownColor := colorize("???   ", colorBold, noColor)
...
func consoleDefaultFormatService(noColor bool) Formatter {
	return func(i interface{}) string {
		var l string
		if ll, ok := i.(string); ok {
			l = colorize(ll, colorBold, noColor)
